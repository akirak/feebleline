...
(defvar feebleline--last-message nil)
...
(defun feebleline--insert ()
  ...
  (let ((left ())
    ...
    (let* ((left-string (string-join (nreverse left)))
           message)
      (if right
          (let* ((right-string (string-join (nreverse right)))
                 (free-space (- (window-width) (length left-string) (length right-string)))
                 (padding (make-string (max 0 free-space) ?\ )))
            (setq message (concat left-string padding right-string)))
        (setq message left-string))
      (unless (string-equal message feebleline--last-message)
        (let ((message-truncate-lines t)
              (max-mini-window-height 1))
          (setq feebleline--last-message message)
